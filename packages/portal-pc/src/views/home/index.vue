...
// import GptxChat2 from '@gptx/components/src/components/GptxChat/index.vue'
const router = useRouter()
const chatApiUrl = computed(() => {
  const storageUserInfo = JSON.parse(localStorage.getItem('userInfo'))
  if (storageUserInfo.data.isAnonymous){ // Anonymous
    return '/portal/conversation/chat-anonymous'
  } else {
    return '/portal/conversation/index-chat'
  }
})
...
const _getChatDetail = async () => {
  ...
  const storageUserInfo = JSON.parse(localStorage.getItem('userInfo'))
  if (storageUserInfo.data.isAnonymous){ // no Anonymous
...
const onErrorHandle =(res)=>{
  if (res && res.code &&  (res.code === 901 || res.code === 902)) { // 游客，触发登录
...
const operationConfig = computed(() => {
  // const storageUserInfo = JSON.parse(localStorage.getItem('userInfo'))
  // if (!storageUserInfo.data.isAnonymous && window.SITE_TYPE && window.SITE_TYPE === '2'){ 
  //   return  { addFile: true  }
  // } else {  
    return { addFile: false  }
  // }
})
</script>
...
<template>
  <div class="h-full transition-all chat-bg">
    <GptxChat v-if="botConfig" :bot-info="botConfig" :operation-config="operationConfig" :chat-api-url="chatApiUrl" :is-debug="false" style="max-width: 100%" @on-error="onErrorHandle" />
  </div>
</template>
