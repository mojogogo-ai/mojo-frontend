<template>
  ...
  >
          ...
          <div v-for="item in publishOptions" :key="item.id" class="pdc-list-item">
            <div class="configure-left">
              <van-checkbox
                ...
                :disabled="(item.id === 'telegram' && !item.telegram_token) || (item.id === 'discard' && !item.discord_token)"
    ...
    <template #footer>
      <van-button type="primary" :loading="loading" :disabled="loading || uncheckedOptions.length === 0" @click="submitForm">
        Publish
      </van-button>
    </template>
...
const afterUpdate = (option) => {
  if (option.token_type === 'telegram') {
    const telegramOption = publishOptions.find(item => item.id === 'telegram');
    if (telegramOption) {
      telegramOption.telegram_token = option.token;
      telegramOption.address = option.address;
    }
  } else if (option.token_type === 'discard') {
    const discordOption = publishOptions.find(item => item.id === 'discard');
    if (discordOption) {
      discordOption.discord_token = option.token;
      discordOption.address = option.address;
    }
  }
};
...
const open = async (option) => {
  ...
  try {
    ...
    if (code === 200) {
     ...
     showToast('Failed to get bot info');
    }
  } catch (e) {
   showToast('Failed to get bot info');
...
const submitForm = async () => {
  ...
  try {
    await botPublish({
      ...
      discord_token: publishOptions.find(item => item.id === 'discard').discord_token,
      address: publishOptions.find(item => item.id === 'telegram').address || publishOptions.find(item => item.id === 'discard').address
    });
...
.pdc-list {
  ...
  .configure-left {
    ...
    .configured {
      ...
      gap: 10px;
      border-radius: 4px;
      background: var(--neon-green-20, rgba(224, 255, 49, 0.2));
      color: #fff;
      font-size: 11px;
      font-weight: 500;
      line-height: 23px;
